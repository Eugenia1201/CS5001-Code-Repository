"""CS 5001 - Kaiqi Zhang - Project 6 'The Forest Awakens'
   - Extension 1 - 26 Oct
"""
...
class Forest:
    ...
    Attributes:
        ...
        A list of Tree objects for the forest background.
        foreground_trees (list):
        A list of Tree objects for the forest foreground.
    ...
    def __init__(self):
        ...
        self.background_trees = []  # Create a list for background trees
        self.foreground_trees = []  # Create a list for foreground trees
        self.families = []

        for _ in range(10):
            x = random.randint(0, window.width - 100)
            y = random.randint(600, window.height-100)
            size = random.uniform(0.8, 1)
            tree = Tree(x, y, size)
            self.background_trees.append(tree)

        for _ in range(10):
            x = random.randint(0, window.width - 100)
            y = random.randint(100, (window.height//2)-150)
            size = random.uniform(1, 1.2)
            tree = Tree(x, y, size)
            self.foreground_trees.append(tree)
    ...
    def draw_background_trees(self):
        """
        Draw the background trees in the forest.
        """
        for tree in self.background_trees:
            tree.draw()

    def draw_foreground_trees(self):
        """
        Draw the foreground trees in the forest.
        """
        for tree in self.foreground_trees:
            tree.draw()

    def draw_families(self):
        """
        Draw the animal families in the forest.
        """
...
class App:
    ...
    def run(self):
        ...
        def on_draw():
            window.clear()
            forest.draw_background_trees()
            forest.draw_families()
            forest.draw_foreground_trees()
