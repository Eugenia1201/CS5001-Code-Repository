"""CS 5001 - Project 10 - Into the Sea World - Extension 1 - Kaiqi Zhang"""
...
def on_draw():
    ...
    if the_seaworld is not None:
        if config_de.light_mode == 1:
            window.clear()
            gl.glClearColor(0.5, 0.7, 1.0, 1.0)
            the_seaworld.draw()
        else:
            window.clear()
            gl.glClearColor(0.2, 0.3, 0.5, 0.8)
            the_seaworld.draw()
...
class App:
    ...
    def run(self):
        ...
        global the_seaworld
        global config_de
        with open("seaworldconfig_extension.json", "r", newline="") as in_file:
