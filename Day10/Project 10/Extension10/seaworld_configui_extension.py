"""CS 5001 - Kaiqi Zhang - Project 10 'Into the Sea World'
   Configui File for Extention 1
"""

import seaworldconfig_extension
...
class App:
    ...
    def __init__(self):
        ...
        global checkbox
        checkbox = IntVar()
        ttk.Checkbutton(my_frame, text="Light Background",
                        variable=checkbox).grid(row=12, column=0)

        ttk.Button(my_frame, text="Save",
                   command=self.on_start).grid(row=13, column=0)
    ...
    def on_start(self):
        ...
        global config
        config = seaworldconfig_extension.SeaworldConfig(config_numbers[0],
                                                         config_numbers[1],
                                                         config_sizes[0],
                                                         config_sizes[1],
                                                         config_numbers[2],
        ...
        with open("seaworldconfig_extension.json", "w") as out_file:
            out_file.write(en)
