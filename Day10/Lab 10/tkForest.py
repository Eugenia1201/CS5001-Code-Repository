...
root = tk.Tk()
window = pyglet.window.Window(width=1000, height=1000, visible=False)
pause = False
global config
global forest
...
class App:
    ...
    def __init__(self):
        ...
        ttk.Button(my_frame, text="Into the forest!",
    ...
    def on_start(self):
        ...
        for i in config_number_strings:
            result = number_or_none(i)  # To remember the result number
            if result is None:
                return
            if result < 3:
            ...
            if result > 20:
                ...
                #  Store the result for later use, maybe append a new list
        ...
        global config
        config = ForestConfig(config_numbers[0], config_numbers[1],
                              config_sizes[0], config_sizes[1],
                              config_numbers[2], config_numbers[3],
                              config_sizes[2], config_sizes[3],
                              config_numbers[4], config_numbers[5],
                              config_sizes[4], config_sizes[5],
                              )
...
class Family:
    ...
    def __init__(self, animal_class):
        ...
        self.kids = []
        num_kids = random.randint(config.nkids_min, config.nkids_max)
        for x in range(num_kids):
            kid = animal_class(800, 800, size=random.uniform(
                config.kid_size_min, config.kid_size_max))
...
class Forest:
    ...
    def __init__(self):
        ...
        num_trees = random.randint(config.ntrees_min, config.ntrees_max)
        for _ in range(num_trees):
            tree = Tree(x=random.randint(10, window.width-200),
                        y=random.randint(200, window.height-250),
                        size=random.uniform(config.tree_size_min,
                                            config.tree_size_max))
...
class Flock:
    ...
    def __init__(self, animal_class, forest):
        ...
        for _ in range(random.randint(config.nflyer_min, config.nflyer_max)):
            random_tree = random.choice(forest.trees)
            x, y = random_tree.perch_x(), random_tree.perch_y()
            flyer = animal_class(x, y, size=random.uniform(
                config.flyer_size_min, config.flyer_size_max))
